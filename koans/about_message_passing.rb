...
class AboutMessagePassing < Neo::Koan
  ...
  def test_methods_can_be_called_directly
    ...
    assert true, mc.caught?
  end

  def test_methods_can_be_invoked_by_sending_the_message
    mc = MessageCatcher.new

    assert true, mc.send(:caught?)
  end
  ...
  def test_methods_can_be_invoked_more_dynamically
    ...
    assert true, mc.send("caught?")
    assert true, mc.send("caught" + "?" )    # What do you need to add to the first string?
    assert true, mc.send("CAUGHT?".downcase )      # What would you need to do to the string?
  end
